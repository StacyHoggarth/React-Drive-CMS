...
class Page extends PureComponent {
  static readyOnActions(dispatch) {
    return Promise.all([dispatch(categoryActions.fetchCategoriesIfNeeded())]);
  }
  ...
  render() {
    ...
    return html`
      <div className=${css(blocks.wrapper, styles.page)}>
        <${Helmet}
          title=${title}
          titleTemplate=${"%s - Stacy and Dell's Place"}
          meta=${[
            { "char-set": "utf-8" },
            { name: "description", content: title },
          ]}
        />
        <button className=${css(styles.menuBurger)} onClick=${this.toggleMenu}>
